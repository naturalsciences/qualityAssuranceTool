...
```bash
docker run --rm --network=host --user "$(id -u):$(id -g)" --workdir /app -v ./config:/app/conf -v ./outputs:/app/outputs -e DEV_SENSORS_USER=$DEV_SENSORS_USER -e DEV_SENSORS_PASS=$DEV_SENSORS_PASS vdsnils/quality_assurance_tool:latest "time.start=2023-06-01" "time.end=2023-07-01"
```
...
```bash
docker buildx build --build-arg ID_U=$(id -u) -t TAG . 
```
